...
- `O(√n)`, корневая √10_000 = 100 операций
- `O(n)`, линейная - 10_000 операций (если 1 операция ≈ 1 ns то время работы ~10 мкс)
- `O(n log n)`, линейно-логарифмическая (quicksort) - 10_000 × 13 ≈ 130_000 (если 1 операция ≈ 1 ns то время работы ~130 мкс)
- `O(n²)`, квадратичная - 100_000_000 операций (если 1 операция ≈ 1 ns то время работы ~0.1 сек)
- `O(n³)`, кубическая (как в тройном вложенном цикле) - 1_000_000_000_000 операций (если 1 операция ≈ 1 ns то время работы ~17 мин)
- `O(2^n)`, экспоненциальная - 2^10 000 ≈ 10^3010 операций (если 1 операция ≈ 1 ns то время работы недостижимо)
- `O(n!)`, факториал 10_000_000_000_000_000_000_000_000_... операций
