...
mod ds_counting_sort{
        ...
        T: Clone,
    {
        let mut prefix_sums = {
            ...
            let len:usize = if max > arr.len(){max+1}else{arr.len()+1};
    ...
    pub fn counting_sort_opt(arr: &mut Vec<i32>){
        ...
        for i in arr.iter(){                      // N
            ...
            alloc3(&mut buff[b[0] as usize][b[1] as usize][b[2] as usize],b[3] as usize);// S
            buff[b[0] as usize][b[1] as usize][b[2] as usize][b[3] as usize]+=1;
        }

        let mut index_set = 0;
        for (index0,v0) in buff.iter().enumerate(){                         // M 
            for (index1,v1) in v0.iter().enumerate(){                            // K
                for (index2,v2) in v1.iter().enumerate(){                             // F
                    for (index3,count) in v2.iter().enumerate().filter(|(_,c)|*c>&0){// G
                        for _ in 0..*count as usize{                                            // H
                            arr[index_set]=i32::from_be_bytes([index0 as u8,index1 as u8,index2 as u8,index3 as u8])+min;
                            index_set+=1;
                        }
                    }
                } 
