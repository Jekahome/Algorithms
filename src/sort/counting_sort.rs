...
///
/// Анализ сложности счетной сортировки:
///     Временная сложность : O(N+M), где N и M — размер inputArray[] и countArray[] соответственно.
///     В худшем случае: O(N+M).
///     Средний случай: O(N+M).
///     В лучшем случае: O(N+M).
///     Вспомогательное пространство: O(N+M), где N и M — это пространство, занимаемое выходными массивами[] и countArray[] соответственно.
///
/// Преимущество счетной сортировки:
///     Сортировка подсчетом обычно выполняется быстрее, чем все алгоритмы сортировки на основе сравнения, такие как сортировка слиянием и быстрая сортировка, если диапазон входных данных имеет порядок количества входных данных.
///     Счетная сортировка легко кодируется
///     Счетная сортировка — устойчивый алгоритм .
///
/// Недостатки счетной сортировки:
///     Сортировка подсчетом не работает с десятичными значениями.
///     Сортировка по подсчету неэффективна, если диапазон сортируемых значений очень велик.
///     Сортировка по подсчету не является алгоритмом сортировки на месте . Она использует дополнительное пространство для сортировки элементов массива.
///
